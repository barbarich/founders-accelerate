...
export default function M2Slide23Step2() {
  const steps = [
    { time: "0–3", action: "Запишите ВСЕ фичи вашего продукта — от самых важных до «было бы круто»" },
    { time: "3–6", action: "Разнесите по матрице: влияние на пользователя × сложность реализации" },
    { time: "6–8", action: "Зачеркните всё из квадрата «Убить». Отложите «Запланировать» и «Может быть»" },
    { time: "8–10", action: "Что осталось? Это ваш MVP. Обсудите с группой — хватит ли этого для первого клиента?" },
  ];
    ...
    return (
      <div className="w-full h-full bg-[hsl(var(--slide-bg))] flex flex-col justify-center px-[24px]">
        ...
        <div className="bg-[hsl(var(--slide-gold)/0.06)] border border-[hsl(var(--slide-gold)/0.2)] rounded-[6px] px-[10px] py-[8px]">
          <p className="text-[10px] text-[hsl(var(--slide-text))] font-semibold">Результат: список из 1-3 фич, которые войдут в MVP</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="w-full h-full bg-[hsl(var(--slide-bg))] flex flex-col justify-center px-[140px]">
      ...
      <h2 className="font-display text-[56px] font-bold text-[hsl(var(--slide-text))] leading-[1.1] mb-[48px]">Убиваем фичи: 10 → 3</h2>
      <div className="space-y-[20px] max-w-[1100px]">
        {[
          ...
          { time: "3–6 мин", action: "Разнесите по матрице: влияние на пользователя (высокое/низкое) × сложность реализации (высокая/низкая)" },
          { time: "6–8 мин", action: "Зачеркните всё из квадрата «Убить». Отложите «Запланировать» и «Может быть» на потом" },
          { time: "8–10 мин", action: "Что осталось? Это ваш MVP. Обсудите с группой — хватит ли этого для первого платящего клиента?" },
        ].map((s, i) => (
      ...
      <div className="mt-[44px] bg-[hsl(var(--slide-gold)/0.06)] border border-[hsl(var(--slide-gold)/0.2)] rounded-[12px] px-[28px] py-[18px]">
        <p className="text-[20px] text-[hsl(var(--slide-text))] font-semibold">Результат: список из 1-3 фич, которые войдут в MVP. Остальное — в бэклог</p>
      </div>
    </div>
  );
}
