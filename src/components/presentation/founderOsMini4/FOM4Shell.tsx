...
import { slides } from "./FounderOsMini4Slides";
import { SlideMetaProvider } from "@/components/mini-course/SlideMetaContext";

const TOTAL = slides.length;
const FOOTER_LABEL = "Михаэль · Сессия 4 из 6";

export default function FOM4Shell({ backTo = "/admin/meetings" }: { backTo?: string } = {}) {
      ...
      <div className="bg-[#0A0E1A] mini-print-root">
        ...
        {slides.map((Slide, i) => (
          <div key={i} className="mini-print-page">
            <SlideMetaProvider value={{ index: i + 1, total: TOTAL, lesson: 4, footerLabel: FOOTER_LABEL }}>
              <Slide />
            </SlideMetaProvider>
          </div>
        ))}
        ...
        <div className={`absolute inset-0 transition-opacity duration-200 ease-in-out ${transitioning ? "opacity-0" : "opacity-100"}`}>
          <ScaledSlide>
            {(() => {
              ...
              return (
                <SlideMetaProvider value={{ index: displayed + 1, total: TOTAL, lesson: 4, footerLabel: FOOTER_LABEL }}>
                  <Slide />
                </SlideMetaProvider>
              );
            })()}
