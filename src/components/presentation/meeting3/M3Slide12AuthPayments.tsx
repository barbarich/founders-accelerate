...
const sections = [
  {
    ...
    options: [
      { label: "Google OAuth", desc: "Минимум трения — 1 клик, самая высокая конверсия регистрации", best: "B2C продукты" },
      { label: "Email + пароль", desc: "Классика, работает везде. Добавьте подтверждение email", best: "Универсально" },
      { label: "Magic link", desc: "Email без пароля — пользователь получает ссылку для входа", best: "Простые продукты" },
    ],
    prompt: "Подключи Supabase Auth. Добавь вход через Google OAuth и email/пароль. На главной — кнопка «Войти» в хедере. После входа — редирект на /dashboard. Незалогиненных пользователей редиректь на /login при попытке открыть защищённые страницы.",
  },
  {
    icon: "💳",
    title: "Приём оплаты",
    ...
    options: [
      { label: "Подписка", desc: "Ежемесячный платёж, автосписание. Нужен webhook для управления статусом", best: "SaaS, сервисы" },
    ...
    prompt: "Подключи Stripe (Израиль — Grow/Meshulam). Создай /pricing с тарифами: Free (до 100 записей) и Pro $29/мес (безлимит). Кнопка «Upgrade» → Checkout. После оплаты — webhook обновляет поле plan в таблице users в Supabase.",
  },
];

export default function M3Slide12AuthPayments() {
