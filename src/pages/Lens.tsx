...
export default function Lens() {
  ...
  // Effective model id sent to backend: custom wins if user typed it and picked __custom__
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        {model !== CUSTOM_MODEL_ID && !modelSupportsSearch(provider as Provider, model) && (
          <p style={{ fontSize: 12, color: "#b45309", marginTop: 6 }}>
            ⚠️ Эта модель без веб-поиска — отчёт будет на знаниях модели, без актуальных источников. Для ресерча с источниками выбери модель с {SEARCH_BADGE}.
          </p>
        )}
        <p style={{ fontSize: 12, color: C.muted, marginTop: 6 }}>
          Дороже модель = качественнее отчёт, но и дольше прогон + больше трат с твоего ключа.
