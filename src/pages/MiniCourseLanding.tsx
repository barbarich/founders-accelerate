    ...
    <header className="mcl-hero">
      <div className="mcl-container">
        ...
        <p className="mcl-lede">
          5 уроков, ~3 часа. От первого интервью до первых платящих клиентов. Не теория из YouTube — мой реальный воркфлоу из живых стартапов с двумя экзитами и 50,000+ платящими пользователями в 107 странах.
        </p>
      ...
      <div className="mcl-container-wide">
        <div className="mcl-founder-strip-inner">
          ...
          <div className="mcl-founder-stats">
            ...
            <div>
              <div className="mcl-founder-stat-num">$3M<em></em></div>
              <div className="mcl-founder-stat-label">ARR одного из продуктов</div>
            </div>
            <div>
              <div className="mcl-founder-stat-num"><em>4×</em></div>
...
function Pain() {
  const items = [
    ["Ты строишь, не поговорив ни с одним реальным пользователем", "«Ну я же сам этим пользуюсь» — это не валидация. Это автобиография."],
    ["Ты не можешь объяснить, для кого продукт, в одном предложении", "Если позиционирование размытое — реклама не работает, продажи не закрываются, контент не попадает."],
    ["Конкуренты гуглятся как «топ-5 в индустрии», а ты — нет", "Не понимая дыр у них, ты строишь дублёра. Дублёров не покупают."],
    ["Ты живёшь в Cursor / Lovable, но воркфлоу хаотичный", "Тратишь часы там, где должны быть минуты. Перепрашиваешь одно и то же по 5 раз."],
    ["Запуск — это пост в Product Hunt и тишина", "Без скрипта холодных продаж и руками собранных первых клиентов запуска не существует."],
  ] as const;
  return (
    <section className="mcl-pain">
      <div className="mcl-container">
        <div className="mcl-section-label">Узнаёшь себя?</div>
        <h2 className="mcl-section-title">Ты строишь продукт. <em>Но не уверен, что его кто-то купит.</em></h2>
        <p className="mcl-section-intro">
          Это самая частая история, которую я вижу у фаундеров вокруг себя. Кто-то ещё на этапе идеи и боится начать. Кто-то открыл Lovable, за выходные собрал MVP — и тишина. Кто-то годами докручивает продукт, который никому не нужен. Вот цифра, которая объясняет почему.
        </p>
        ...
        <ul className="mcl-pain-list">
          {items.map(([t, s]) => (
            <li key={t}><strong>{t}</strong><span>{s}</span></li>
          ))}
        </ul>
      </div>
    </section>
        ...
        <div className="mcl-about-card">
          ...
          <div>
            ...
            <p className="mcl-about-quote">
              «Команда из 12 человек делала за месяц то, что я сейчас делаю один за неделю с AI. И при этом мои продукты лучше, чем были тогда.»
            </p>
            <ul className="mcl-about-bullets">
              ...
              <li><span><strong>ForexTester</strong> — финтех-продукт. Дошёл до $3M ARR, продан.</span></li>
              <li><span><strong>MetaMinder</strong> — B2B-LMS для frontline-обучения. Клиенты на всех континентах.</span></li>
              <li><span><strong>Mikey AI</strong> — AI-приложение для матчинга. Построил один, без программирования, с нуля.</span></li>
              <li><span><strong>The Founders Circle</strong> — менторская программа для фаундеров. Первая когорта поставила <strong>10/10</strong>.</span></li>
            </ul>
          </div>
        </div>
      </div>
...
const LESSONS = [
  ...
  { num: "01", title: "Customer Development: 10 разговоров до первого пикселя", meta: "~50 минут · урок 1", body: "Главный кейс — мой провал InterviewNinja: 18 месяцев, потерянные деньги, ошибки, которые я бы не повторил. Ты получаешь playbook на 10 правильных интервью: кого выбирать, как формулировать вопросы (без наводящих), pass-сигналы и fail-сигналы. И почему «звучит интересно» — это всегда нет.", result: "Список 10 интервьюируемых, скрипт интервью, чек-лист сигналов. Можешь начинать разговоры в этот же вечер." },
  { num: "02", title: "Research: что НЕ строить", meta: "~50 минут · урок 2", body: "Главный кейс — Mikey AI: как я выбирал нишу в дейтинге, где «всё уже есть». Ты делаешь карту конкурентов с AI за 90 минут, находишь дыры в их офферах и формулируешь ОДНО узкое позиционирование, которое отличает тебя от 50 клонов на рынке.", result: "Готовая карта конкурентов и одна точная формулировка позиционирования, которой ты можешь объяснить продукт за 10 секунд." },
  { num: "03", title: "Разработка с AI: воркфлоу, а не инструменты", meta: "~60 минут · урок 3", bodyHtml: 'Главный кейс — Mikey, построенный без программистов. Ты получаешь мой реальный воркфлоу agentic engineering: design doc → промпт → ревью diff → тесты. Как работать с Claude Code, Cursor и Lovable так, чтобы получать MVP за дни, а не за недели. Плюс <strong>сегмент «лендинг за 30 минут»</strong> — где я строю лендинг руками, в реальном времени, по моим промптам.', result: "Рабочий воркфлоу разработки + готовый лендинг (или MVP-фича) к концу урока, построенный по тем же шагам." },
  { num: "04", title: "Запуск и первые 10 платящих клиентов", meta: "~50 минут · урок 4", body: "Главный кейс — RunEverywhere: viral-момент, который дал нам первых 10,000 пользователей без рекламы. Запуск — это не Product Hunt и тишина. Это руками собранные 10 первых клиентов через скрипты холодной рассылки, pre-sale до полноценной разработки и формула лендинга, которой я пользуюсь сейчас в 4 продуктах.", result: "Скрипт холодной рассылки, pre-sale стратегия и понимание, как закрыть первого платящего клиента до полноценного запуска." },
] as const;
  ...
  return (
    <section className="mcl-bonus" id="bonuses">
      <div className="mcl-container">
        ...
        <p className="mcl-section-intro">
          $19 — это абсурдно дёшево, и я знаю почему. Я хочу, чтобы вход был импульсной покупкой, а не решением, которое ты обдумываешь две недели. Поэтому в курс я включил всё, чем пользуюсь сам.
        </p>
        ...
        <div className="mcl-value-summary">
          <div className="mcl-value-row"><span>Видеокурс (5 уроков, ~3 часа)</span><strong>$300</strong></div>
...
const CASES = [
  ...
  { tag: "УРОК 3 · РАЗРАБОТКА", name: "MetaMinder", stat: "B2B LMS · клиенты на всех континентах", lesson: "как с командой 12 человек делалось то, что я сейчас делаю один за неделю — и почему AI здесь не магия, а воркфлоу." },
  { tag: "УРОК 4 · ЗАПУСК", name: "RunEverywhere", stat: "B2C · 50,000+ платящих в 107 странах", lesson: "viral-момент, который дал первых 10,000 пользователей без рекламы — и как его повторить руками, а не ждать удачи." },
] as const;
...
const TESTIMONIALS = [
  { text: "«Михаэль не сглаживает углы — он сразу говорит, где ты тратишь время впустую. Эта прямота сэкономила мне минимум 6 месяцев. К 8-й неделе у меня был первый платящий клиент.»", a: "AL", n: "Amit Levi", r: "SaaS founder · Tel Aviv" },
  { text: "«Лучшая часть — не теория, а то, что Михаэль заставляет реально делать работу прямо в моменте. К 4-й неделе у меня были первые платящие клиенты. Я проходил другие программы. Эта другая, потому что тут негде спрятаться.»", a: "TH", n: "Tomáš Horák", r: "Marketplace startup · Prague" },
  { text: "«Я был скептиком — ещё одна программа? Но это не курс. Это структурированный путь от нуля до запуска с человеком, который реально это сделал несколько раз. У моего стартапа сейчас 40+ пользователей, и на прошлой неделе я закрыл первый годовой контракт.»", a: "OM", n: "Oleg Marchenko", r: "B2B SaaS · ранний этап" },
  { text: "«Я была в других акселераторах. Этот другой, потому что Михаэль реально следит за прогрессом неделя за неделей. Он помнил детали моего продукта, которые я упоминала вскользь. Одна только методология custdev уже окупила инвестицию.»", a: "YB", n: "Yael Ben-David", r: "HealthTech founder" },
] as const;
          ...
          {TESTIMONIALS.map((t) => (
            <div className="mcl-testimonial" key={t.n}>
              ...
              <div className="mcl-testimonial-author">
                <div className="mcl-testimonial-avatar">{t.a}</div>
      ...
      <div className="mcl-container">
        ...
        <div className="mcl-pricing-card">
          ...
          <ul className="mcl-pricing-features">
            <li>5 уроков (вводный + 4 полноценных), ~3 часа видео</li>
          ...
          <div className="mcl-guarantee">
            ...
            <div className="mcl-guarantee-text">
              ...
              Если в течение недели поймёшь, что курс не для тебя — верну деньги. Без вопросов, без анкет, без переписки.
            </div>
          </div>
        </div>
      </div>
...
const FAQ_ITEMS = [
  ...
  { q: "Сколько времени занимает курс?", a: ["~3 часа видео + примерно столько же на практику внутри уроков. То есть 6 часов на полный проход. Можно за один уикенд, можно растянуть на 2 недели — материал не устаревает.", "Доступ остаётся у тебя навсегда, плюс все будущие обновления."] },
  ...
  { q: "Что если мне не подойдёт?", a: ["7 дней на возврат, без вопросов. Просто пишешь мне на почту — я возвращаю $19. Никаких форм, обоснований и удерживающих звонков.", "Я делаю это потому что уверен в материале. Но если он не для тебя — не хочу держать твои деньги."] },
...
function FinalCTA() {
  return (
    <section className="mcl-final-cta">
      <div className="mcl-container">
        <h2>Через 3 часа у тебя будет <em>другая голова</em></h2>
        <p>
          Не «больше знаний». Другая голова — которая видит, что в твоём продукте сломано, и точно знает, что делать дальше. За $19 это, возможно, самая короткая дорога, которую я могу тебе предложить.
        </p>
        <a href="#buy" className="mcl-cta-primary">Купить курс за $19</a>
        <div className="mcl-hero-meta">
          <span className="mcl-hero-meta-item">{CHECK_ICON} Возврат 7 дней без вопросов</span>
          <span className="mcl-hero-meta-item">{CHECK_ICON} Доступ навсегда</span>
        </div>
      </div>
    </section>
