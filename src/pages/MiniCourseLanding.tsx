...
function Pain() {
  const pairs = [
    {
      before: ["Ты строишь, не поговорив ни с одним реальным пользователем", "«Ну я же сам этим пользуюсь» — это не валидация. Это автобиография."],
      after: ["Ты говоришь с реальными людьми и слышишь их боль", "5–10 интервью по моему скрипту, и ты точно знаешь, за что готовы платить, а что — мимо."],
    },
    {
      before: ["Ты не можешь объяснить, для кого продукт, в одном предложении", "Если позиционирование размытое — реклама не работает, продажи не закрываются, контент не попадает."],
      after: ["У тебя есть одно предложение, от которого «щёлкает»", "Чёткий сегмент, чёткая боль, чёткое обещание. Реклама, продажи и контент начинают работать как одна система."],
    },
    {
      before: ["Конкуренты гуглятся как «топ-5 в индустрии», а ты — нет", "Не понимая дыр у них, ты строишь дублёра. Дублёров не покупают."],
      after: ["Ты знаешь, где конкуренты слабые, и бьёшь именно туда", "Понимаешь рынок на уровне отзывов и реальных жалоб — и строишь не дублёра, а альтернативу, которую выбирают."],
    },
    {
      before: ["Ты живёшь в Cursor / Lovable, но воркфлоу хаотичный", "Тратишь часы там, где должны быть минуты. Перепрашиваешь одно и то же по 5 раз."],
      after: ["У тебя есть мой воркфлоу agentic engineering", "Design doc → промпт → ревью diff → тесты. Фичи, которые делались неделями, выходят за часы."],
    },
    {
      before: ["Запуск — это пост в Product Hunt и тишина", "Без скрипта холодных продаж и руками собранных первых клиентов запуска не существует."],
      after: ["У тебя есть скрипт, лендинг и первые платящие до запуска", "Холодная рассылка, pre-sale, упаковка — ты подходишь к запуску с деньгами и обратной связью на руках."],
    },
  ] as const;
  return (
      ...
      <div className="mcl-container">
        ...
        <div className="mcl-ba-grid">
          ...
          {pairs.map(({ before, after }) => (
            <div key={before[0]} className="mcl-ba-row">
