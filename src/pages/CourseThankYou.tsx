...
// Fallback only — the real tier price ($397 or $997) is captured at
// begin_checkout on the landing and read via getCheckoutValue().
const FALLBACK_PRICE_USD = 397;

/** Stable transaction id: Stripe session_id → stored event_id → synthetic. */
...
export default function CourseThankYou() {
  useEffect(() => {
    document.body.classList.add("mcl-body");
    const prevTitle = document.title;
    document.title = "Спасибо за покупку — большой курс";
  ...
  return (
    <div className="mcl-root" style={{ minHeight: "100vh", paddingTop: 0 }}>
      <SEO
        path="/course/thank-you"
        title="Спасибо за покупку - большой курс"
        description="Подтверждение покупки большого курса. Дальше - доступ через Telegram-бота."
        noindex
      />
        ...
        >
          ...
          >
            ...
            <p style={{ marginTop: 14, marginBottom: 0, fontSize: 14, color: "var(--mcl-muted, #666)", lineHeight: 1.6 }}>
              Купил тариф с консультациями? После открытия курса в боте появится кнопка записи на 3 личные встречи со мной.
            </p>
          </div>
