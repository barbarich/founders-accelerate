  ...
  return (
    <div className="min-h-screen" style={{ background: "#f8f8f4", color: "#0d0d0d", fontFamily: "var(--nl-font-body, 'DM Sans', sans-serif)" }} dir={isRtl ? "rtl" : "ltr"}>
      ...
      <main className="max-w-[900px] mx-auto px-6 lg:px-10 py-12 md:py-20">
        <h1 className="text-3xl md:text-4xl mb-3" style={{ fontFamily: "var(--nl-font-display, 'Syne', sans-serif)", fontWeight: 800 }}>{t.termsTitle}</h1>
        <p className="text-sm mb-12" style={{ color: "#aaa" }}>{t.termsLastUpdated}</p>
      ...
      <footer className="py-8" style={{ borderTop: "1px solid #e4e4dc" }}>
        <div className="max-w-[900px] mx-auto px-6 lg:px-10 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <Link to={`/${lang}/privacy`} className="text-xs underline underline-offset-2 transition-colors" style={{ color: "#aaa" }}>{t.footerPrivacy}</Link>
          ...
          <div className="flex items-center gap-1">
            {supportedLangs.map((l) => (
              <Link key={l} to={`/${l}/terms`} className="px-2 py-1 text-[11px] rounded transition-colors" style={{ color: l === lang ? "#4a8a28" : "#aaa" }}>
                {langLabels[l]}
              </Link>
            ))}
          </div>
